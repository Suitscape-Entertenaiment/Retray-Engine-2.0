...
import { axis, grid, camera, controls, cam3d_third_person_fn } from "../3d/core3d";
...
const oc = document.getElementById("optionalColor");
const datos_curiosos = ["Quiero mis Vacaciones Sr Pool...", "Prueben NewCatroid!"];
const indice = Math.floor(Math.random() * datos_curiosos.length);
const intervalId = setInterval(() => {
    if (pyodide) {
        if (ltxt)
            ltxt.textContent = "¡Listo!";
        if (loader) {
        ...
        if (ltxt)
            ltxt.textContent = datos_curiosos[indice];
    }
}, 300);
...
async function ejecutar() {
    ...
    try {
        ...
        pyodide.globals.set("move3d", (nombre, x, y, z) => objectList3d[nombre].object.position.set(x, y, z));
        pyodide.globals.set("rotate3d", (nombre, x, y, z) => objectList3d[nombre].object.rotation.set(x, y, z));
        pyodide.globals.set("scale3d", (nombre, x, y, z) => objectList3d[nombre].object.scale.set(x, y, z));
        ...
        pyodide.globals.set("cam3d_third_person", (obj, offsett) => cam3d_third_person_fn(objectList3d[obj], offsett));
        ...
        pyodide.globals.set("playAudio", (name) => audioList[name].play());
        pyodide.globals.set("pauseAudio", (name) => audioList[name].pause());
